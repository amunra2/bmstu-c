...
START_TEST(test_sort_random)
{
    ...
    node_t *new_list = sort(list1, comparator);
...
START_TEST(test_sort_sorted_back)
{
    node_t *list1 = NULL;
...
Suite *reverse_suite(Suite *s)
{
    ...
    tcase_add_test(tc_pos, test_sort_equal);
    tcase_add_test(tc_pos, test_sort_sorted_back);
    tcase_add_test(tc_pos, test_sort_one_el);
    tcase_add_test(tc_pos, test_sort_two_els);

    suite_add_tcase(s, tc_pos);

    tc_neg = tcase_create("negatives");

    tcase_add_test(tc_neg, test_sort_no_els);
